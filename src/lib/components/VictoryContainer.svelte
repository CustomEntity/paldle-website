<script lang="ts">
    ...
    function formatVictoryText(text: string, values: Record<string, string>): string {
        text = text.replace(/<name>{([^}]+)}<\/name>/g, (_: string, value: string) => {
            return `<span class="highlight">${values.name || value}</span>`;
        });

        text = text.replace(/<tries>(.*?)<\/tries>/g, (_: string, content: string) => {
            const processedContent = content.replace(/{([^}]+)}/g, (__: string, key: string) => values[key] || key);
...
<div
    ...
    >
        ...
        {#if imageUrl}
            <div class="flex gap-3 justify-center items-center pt-3 pb-4 pal-title">
                <img
                        src="{imageUrl}"
        ...
        <div class="flex flex-col w-full items-center gap-1 pal-title pb-2 pt-4">
