<script lang="ts">
    ...
    import { useSilhouetteGameData } from "$lib/stores/palGameData.svelte";
    ...
    const { data } = $props();
    const gameData = useSilhouetteGameData();
    let isWon = $state(false);
    let wonEl: HTMLDivElement | undefined = $state();

    const shareText = (id: number, tries: number) => locale.t("pages.silhouette.share.text", { id, tries });
    const copyText = (id: number, tries: number) =>
        locale.t("pages.silhouette.share.to_copy", { id, tries, url: "https://" + WEBSITE_URL });
    ...
    async function handleSelect(name: string) {
        ...
        if (pal.id === daily.pal.id) {
            ...
            gameData.win();
            notifyModeComplete("silhouette", { won: true, attempts: gameData.tries.length });
        }
    }
...
<div class="flex flex-col items-center w-full gap-6">
    <PalToolsBar mode="silhouette" gameData={gameData.data} patchNotes={data.patchNotes} />
    ...
    <div class="flex flex-col items-center w-full md:max-w-[560px] gap-5">
        ...
        {:then [pals, daily, yesterday]}
            {#if !daily}
                <p class="text-white text-lg">{locale.t("pages.silhouette.errors.no_daily")}</p>
            {:else}
                <div class="pal-frame w-full p-6 flex flex-col gap-3 items-center text-center">
                    <span class="pal-title text-xl uppercase">{locale.t("pages.silhouette.title")}</span>
                    <div class="w-52 h-52 flex items-center justify-center">
                        <img src={palImg(daily.pal.icon)} alt={isWon ? daily.pal.name : "?"}
                             class="w-full h-full object-contain transition-all duration-700"
                             style="filter: {isWon ? 'none' : 'brightness(0)'}; {isWon ? '' : 'opacity:0.92;'}" />
                    </div>
                    {#if !isWon}
                        <span class="text-white/60 text-sm">{locale.t("pages.silhouette.hint")}</span>
                    {/if}
                </div>
                ...
                {#if !isWon}
                    <AnswerInput
                        ...
                        placeholder={locale.t("pages.silhouette.placeholder")}
